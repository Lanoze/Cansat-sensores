/*
  ...
  ARQUITETURA: PRODUTOR-CONSUMIDOR (PRÉ-ALOCAÇÃO + CÓDIGO DE ERRO + HARD RESET)
  FORMATO CSV: Padrão Internacional (Delimitador: Vírgula / Decimal: Ponto)
  PROTEÇÃO SD: Pré-Alocação de 400MB (Áudio) e 2MB (CSV) com Diagnóstico de Hardware
*/
...
void setup() {
  ...
  if (sdOk) {
    ...
    // Abre o CSV globalmente e aplica a pré-alocação de 2MB
    ...
    // Abre o arquivo de áudio e aplica a pré-alocação física de 400MB
    ...
    if (audioFile) {
      ...
      tamanhoDadosAudio = 400ULL * 1024ULL * 1024ULL; 
      
      if (audioFile.preAllocate(400ULL * 1024ULL * 1024ULL)) {
        Serial.println("-> [SUCESSO] 400MB reservados fisicamente no cartao!");
      } else {
        Serial.println("-> [AVISO] Falha na pre-alocacao de audio. Prosseguindo normalmente.");
      }
  ...
  Serial.println("Com pré-alocação de memória e diagnóstico de erro");
...
void sdManagerTask(void *pvParameters) {
  ...
  while(1) {
    ...
    // CAPTURA DE ERRO E HARD RESET DE EMERGÊNCIA
